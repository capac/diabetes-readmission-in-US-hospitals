...
def roc_curve_plot_with_auc(rates_dict, work_dir):
    ...
    for ax, (name, rates_list) in zip(axes, rates_tuples):
        ...
        ax.set_title('ROC curve for {0:s}'.format(name.lower()), fontsize=9)
        ax.legend(loc='lower right', fontsize=8)
    fig.suptitle('Receiver Operating Characteristics Curves', fontsize=10)
    fig.tight_layout()
...
def conf_mx_heat_plot(cm_dict, work_dir):
    ...
    for ax, (model_name, conf_mx) in zip(axes, cm_tuples):
        ...
        plt.setp(ax.get_xticklabels(), ha="center", rotation_mode="anchor", rotation=0, fontsize=8)
        plt.setp(ax.get_yticklabels(), ha="center", rotation_mode="anchor", rotation=90, fontsize=8)
        plt.tick_params(which='both', bottom=False, left=False)
        cbar = ax.collections[0].colorbar
        cbar.ax.tick_params(labelsize=8)
        ax.set_title(f'Heatmap for {model_name.lower()}', fontsize=9)
        ax.set_xlabel('Predicted values', fontsize=8)
        ax.set_ylabel('Actual values', fontsize=8)
    fig.suptitle('Confusion matrix heatmaps', fontsize=10)
    fig.tight_layout()
